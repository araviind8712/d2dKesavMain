...
export class NavbarComponent implements OnInit{
  ...
  company(route:any): void {
    ...
    this.isCompany = true;
    this.isHome = false;
    window.location.replace('/d2dKesavMain/'+route+'?page='+route);
  }
  ...
  home(): void {
    ...
    this.isCompany = false;
    this.isHome = true;
    window.location.href = '/d2dKesavMain/home?page=home';
  }
  ...
  redirect(image: string): void {
    try {
      ...
      if (segments.length > 2) {
        ...
        window.location.href = `/d2dKesavMain/services?service=${page}&page=service`;
      }
    } catch (e) {
